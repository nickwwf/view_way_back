...
'''源码地址：https://github.com/stlehmann/Flask-MQTT，
加入上下文管理
'''
"""Flask-MQTT Package.
...
class Mqtt:
    ...
    def init_app(self, app: Flask, config_prefix: str = "MQTT", name: str = "default") -> None:
        """Init the Flask-MQTT addon."""

        if self.app is None:
            self.app = app

        config = app.config[config_prefix]

        if name not in config.keys():
            raise ValueError("Unknown configuration name '{}'".format(name))

        self.client_id = config[name].get("client_id","")
        self.broker_url = config[name].get("host", "localhost")
        self.broker_port = config[name].get("port", 1883)
        self.username = config[name].get("username", None)
        self.password = config[name].get("password", None)
        if "keepalive" in config[name]:
            self.keepalive = config[name]["keepalive"]
        # set client
